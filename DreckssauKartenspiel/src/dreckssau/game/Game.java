...
public class Game implements IDreckssauHandler {
     ...
     *
     * @param difficulty decides witch difficulty should be set for the AI.
     */
     ...
     *
     * @param name sets an Name for the player.
     */
    ...
    private String nextRound() {
        ...
        out += this.getRound();
        out = getStringNextRound(out);
        return out;
    }

    private String getStringNextRound(String out) {
     ...
     * player.
     *
     * @param i amount of tricks to bid or card to play
     */
    ...
    private String getString(String out, boolean isOver) {
        if (isOver) {
    ...
    private String doNextStep() {
        ...
        buffer.makeDecision(this.round.phase.getPossibleActions(), this.players, this.getGamePhase(), this
                .roundNumber, this.round.getTrickCards());
        String out = "";
        try {
            boolean isOver = false;
            ArrayList<Integer> list = round.phase.getPossibleActions();
            if (list.size() != 1) {
                out += round.placeNext(buffer.makeDecision(this.round.phase.getPossibleActions(), this.players, this
                        .getGamePhase(), this.roundNumber, this.round.getTrickCards()));
            } else {
